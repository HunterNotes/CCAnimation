...
- (void)setLevel:(float)level {
    ...
    if (_level != level) {
        ...
        if (!_iconFull) {
            ...
            NSLog(@"ZView：找不到属性iconFull对象，将使用默认风格图标！");
        }
        [self setNeedsDisplay];
    }
}
...
- (void)scoreAnimation {
    ...
    // 动画：UIView动画里面改变 label的center、alpha,
    CGFloat wide = self.bounds.size.width / (CGFloat)_maxLevel;
    CGFloat x = (ceilf(_level) - 0.5) * wide;
